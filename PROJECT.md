...
- **Current task:** **P1-T03**, Anthropic and Google native providers (owner: aider).
- **Next task:** P1-T04, the sentence chunker (owner: aider).
...
- **Toolchain:** TypeScript 7.0.2, Vite 8, Vitest 5, oxlint 1.81, React 19.2, Node 22, **pnpm 12.3.4** (was 11.24.0; pnpm 12 rewrites its own pin, so the bump was taken rather than fought — the Windows box needs pnpm 12 before its next install), axum 0.8, **AI SDK `ai` 7.0.97 + `@ai-sdk/openai-compatible` 3.0.47 in `packages/providers` (P1-T02, ADR-04)**. **rustc 1.98** on the Linux box, 1.97 on Windows. Verified facts in `docs/SURFACE.md`.
- **Test counts (2026-09-11):** 241 TypeScript (P1-T01 machine + P1-T02 LLM provider suites), 17 Rust. `pnpm gate` green. CI was **red on main** from 97dc688 until 12d6da8 — rustc 1.98's new `chunks_exact_to_as_chunks` clippy lint, on a docs-only commit, on both runners.
