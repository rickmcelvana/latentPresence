...
- **Current task:** **P0-T08 Spike E — both legs run (2026-09-11).** Linux **no-go for Tauri**, Windows **go**. One cosmetic row outstanding (a native notification raised from Rust, one tray click) which gates nothing. Ready for P0-T09 once that is ticked.
...
- [ ] **One tray click on Windows**, whenever convenient: tray icon → **Send a test notification**, and say whether a toast actually appeared. The Rust call already returns `Ok`; that is not the same claim as Windows drawing something, because Focus Assist and per-app settings suppress after the API succeeds. Fills the last row of Spike E's table. **Gates nothing** — P0-T09 does not need it.
