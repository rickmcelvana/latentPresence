...
- **Current task:** **P1-T02**, the OpenAI-compatible LLM provider and model discovery, porting the verified shapes from latentCreate `crates/llm-bridge` rather than re-deriving them.
- **Next task:** P1-T03, Anthropic and Google native providers (owner: aider).
...
- **Test counts (2026-09-11):** 217 TypeScript (P1-T01 added the machine and transition suites), 17 Rust. `pnpm gate` green. CI was **red on main** from 97dc688 until 12d6da8 — rustc 1.98's new `chunks_exact_to_as_chunks` clippy lint, on a docs-only commit, on both runners.
