...
- **Current task:** **P0-T05 Spike B** — harness built and wired, waiting on Rick's frame-rate run.
- **Next task:** P0-T06 Spike C, once it is unblocked (see the backlog).
- **Blockers:** **Spike C cannot start**: no `.env` and no Docker on this box, so neither the tunnel nor the CI container is reachable. Spike B's numbers need a visible browser window, which a hidden pane cannot give.
...
- **Avatar stack (Spike B, 2026-09-09):** three 0.185.1 + `@pixiv/three-vrm` 3.5.5 + `@react-three/fiber` 9.7.0 resolve and build under React 19.2.8 (r3f peer is `>=19 <19.3`). The VRM and VRMA both load, 230–730 ms warm. **wawa-lipsync emits fifteen Oculus visemes, not VRM's five** — the mapping lives in `packages/avatar` with tests. Frame rate is unmeasured. `docs/spikes/B-vrm-lipsync.md`.
...
- [ ] **Run `/spike/avatar`** (P0-T05 go/no-go): `pnpm dev`, then http://localhost:5173/spike/avatar, **in a normal Chrome window** — a hidden or throttled pane does not render, so there are no frames to count. Accept, let it settle, press **Take the reading**, then switch GPU preference and take a second. Then drive the mouth from the microphone and say whether it reads as speech or as chewing. Screenshot for the write-up. Steps in `docs/spikes/B-vrm-lipsync.md` under "Outstanding".

- [ ] **Unblock Spike C (P0-T06)**, either one: write `.env` from `.env.example` with the real `DATABASE_URL` for the MariaDB at `10.0.0.1`, **or** install Docker Desktop so the `companion/dev/` compose file can stand a MariaDB 11.8 up locally. Neither exists on this box today, so the spike cannot produce a number.
...
- [ ] P0 Foundations and spikes — T01, T02, T02b, T03, T04, T07 done; B built and awaiting its run; C blocked; E and the retrospective to go
