...
- **Current task:** **P1-T05**, TTS providers (owner: aider).
- **Next task:** P1-T06, STT providers (owner: aider).
...
- **Test counts (2026-09-12):** 318 TypeScript (P1-T01 machine, P1-T02/T03 LLM provider, P1-T04 chunker suites), 17 Rust. `pnpm gate` green. CI was **red on main** from 97dc688 until 12d6da8 — rustc 1.98's new `chunks_exact_to_as_chunks` clippy lint, on a docs-only commit, on both runners.
...
- **Sentence chunker (P1-T04):** `packages/core/chunker` splits a token stream into TTS-sized chunks and lifts `[emote:x]`/`[gesture:x]` out into tags. **Nothing calls it yet** — P1-T05 (TTS) and P1-T11 (transcript) are what consume it. Two things it inherits forward: `maxChars: 200` is a starting value and **P1-T05 measures the real one against TTS latency**; and **ADR-23 (proposed) defers the tag protocol to P1-T12**, which must close that ADR explicitly rather than inherit it. Tag offsets index the *spoken* text, because P2 schedules against TTS word timestamps.
