...
**Amended 2026-09-25 (P3-T07), protocol change, additive:** `PromptContext` gains `userAffect: UserAffect | null` (default null): how the user seems, fused from their words, the model's `[user:x]`, their voice and their face, rendered by core as **one line** — a phrase, where it comes from, and "do not point it out unless they do" — and only at confidence ≥ 0.35 and not neutral, so a null or weak reading leaves the prompt byte-identical. `PROTOCOL_VERSION` stays 1. The engine takes the same estimate in as `affect.user.updated`, **once per user turn** (again only if the model's own read changes the label), never per camera frame.
...
## ADR-34 Face reading: MediaPipe pinned below its telemetry (accepted 2026-09-25)
...
**Decision (accepted by Rick 2026-09-25, D-35): (a), with (b)'s guard as well.**
