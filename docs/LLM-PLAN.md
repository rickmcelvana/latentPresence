...
### P2-T06 Video-call layout and user PiP — owner: sub (done 2026-09-23; `/chat` is the call — `apps/web/src/call/`; R-14 passed)
...
### P2-T07 Tag to animation bridge — owner: main (built 2026-09-23; `packages/avatar/src/cues`, `src/mappings`; R-16 passed (D-28); done-when met for start-of-sentence tags, mid-sentence waits on P2-T09)
...
### P2-T09 Kokoro word timings — owner: main (done 2026-09-23; `packages/ml-web/src/kokoro/word-timings.ts`, `timed-stream.ts`)
...
**Met:** 100 of 110 words (91%), median 31 ms, against 35% for the estimate; the browser's WebGPU worker timed 48 of 48 words of a live reply. **As built:** kokoro-js drops `durations`, so its model call is wrapped; words are aligned to phoneme groups by a DP (espeak merges and expands words); starts follow hexgrad's `join_timestamps` (−3 frames, split spaces, stress marks count), without which every word was ~130 ms late. The largest misses were checked by level and are Whisper's — it starts a post-pause word at the pause's head — so 91% is a floor. The catalog now names the timestamped repo: audio bit-identical, new consent.

### P2-T08 Speak typed replies — owner: main (done 2026-09-23; `ChatSession.setVoice`, `apps/web/src/voice/speaker.ts`; R-17 passed (D-29))
...
**Met** in the Browser pane against `glm-5.2:cloud`: 13.2 s of Kokoro voice, the lip-sync analyser reading it (peak 255 speaking, 0 silent), the talking clip's hands moving; Stop mid-story silent within 150 ms, the line cut at "Some nights it was kind," and her next reply knew the ending was lost. **As built:** a typed turn gets the `Reply` a call uses, so tags act too; Stop follows the heard rule once audible and the typed rule before. Consent asks for Kokoro alone; a server voice needs no WebGPU. Starting a call ends it; it does not survive a reload. The live check found `tapAnalyser.disconnect` throwing after the graph closed — which blanked `/chat`, and End call shared it since P2-T06 — fixed in cfc2a61.
...
### P3-T04 User affect from text — owner: main (done 2026-09-24; `packages/core/src/affect/user-text.ts`, ADR-32 accepted)
...
### P3-T09 Affect in the call — owner: main (done 2026-09-24; R-20 passed, D-32)
