...
**Status: Linux leg done. No-go for Tauri on Linux — `navigator.gpu` does not exist in
WebKitGTK, so ADR-07's existing fallback clause fires and Linux ships as
"companion + Chrome". Windows/WebView2 is not measured yet and this spike does not close
until it is.**
...
| Windows / WebView2 | **not measured — second sitting** | | |
...
- **Windows / WebView2 is not measured.** It needs Rick back on Windows. The shell exists
  now, so it is a run rather than a build. The spike does not close until that column is
  filled in.
- **Tray and notifications** are half-answered: the Notification *API* is present in both
  containers, but no tray icon was built and no notification was raised. Both are Rust-side
  Tauri features, and building them to test a shell that Linux will not ship would have
  been work in the direction the answer already ruled out.
